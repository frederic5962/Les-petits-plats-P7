...
import linearSearch from '../utils/searchLinear.js';
...
import { handleSelectionClick, selectedTags } from '../utils/handleClick.js';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function performMainSearch(searchTerm, exactMatch = false) {
    ...
    searchResults = linearSearch(searchResults, searchTerm); // Utiliser l'algorithme linéaire pour la recherche
  ...
  searchInputs.forEach((input, index) => {
    ...
    clearButton.addEventListener('click', () => {
      input.value = '';
      clearButton.style.display = 'none'; // Masquer le bouton après effacement
      input.dispatchEvent(new Event('input')); // Pour réinitialiser les résultats
      resetSearchResults(); // Réinitialiser les résultats de recherche
    });
